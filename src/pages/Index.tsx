...
const Index = () => {
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
            ...
            <div>
              ...
              <div className="relative">
                ...
                <Input
                  className="pl-9"
                  value={faturamento}
                  onChange={(e) => setFaturamento(maskMoney(e.target.value))}
            ...
            <div className="flex items-end">
              ...
              >
                {loading ? "Consultando..." : (
                  <>
                    Analisar <ArrowRight className="w-4 h-4 ml-1" />
                  </>
                )}
              </Button>
            </div>
              ...
              <div className="col-span-2 md:col-span-4 pt-3 border-t border-white/10 grid sm:grid-cols-2 gap-4">
                <div>
                  <div className="text-[10px] uppercase tracking-wider text-white/60">
                    Crédito potencial conservador (5 anos)
                  </div>
                  <div className="text-2xl font-bold text-accent-light mt-1">
                    R$ {Math.round(resumo.creditoCons).toLocaleString("pt-BR")}
                  </div>
                </div>
                <div>
                  <div className="text-[10px] uppercase tracking-wider text-white/60">
                  ...
                  <div className="text-2xl font-bold text-accent-light mt-1">
                    R$ {Math.round(resumo.creditoOt).toLocaleString("pt-BR")}
                  </div>
                </div>
              </div>
            )}
        ...
        <section className="container mt-4">
          <div className="bg-card border border-border rounded-lg p-5">
            ...
            <div className="text-sm text-foreground leading-relaxed whitespace-pre-line">
              {analise || (
                <span className="text-muted-foreground italic">Gerando análise…</span>
              )}
            </div>
          </div>
        </section>
          ...
          <div className="flex flex-wrap gap-3 mb-4 items-center">
            ...
            <Select value={fMat} onValueChange={setFMat}>
              ...
              <SelectContent>
                ...
                {(["Pacificada","Em afetação","Controvertida","Desfavorável","Regulatória"] as Maturidade[]).map((m) => (
            ...
            <Select value={fArea} onValueChange={setFArea}>
              ...
              <SelectContent>
                ...
                {(["PIS/COFINS","ICMS","IRPJ/CSLL","IPI","Previdenciário","Subvenções","Reforma Tributária"] as Area[]).map((a) => (
            ...
            <Select value={fRisco} onValueChange={setFRisco}>
              ...
              <SelectContent>
                ...
                {(["Baixo","Baixo-médio","Médio","Alto","Regulatório"] as Risco[]).map((r) => (
                  <SelectItem key={r} value={r}>{r}</SelectItem>
                ))}
              </SelectContent>
            </Select>
            <span className="text-xs text-muted-foreground ml-auto">
              {tesesFiltradas.length} de {teses.length}
            </span>
          </div>
      ...
      {/* Estado vazio */}
      {!empresa && !loading && (
        <section className="container py-16 flex-1 grid place-items-center">
          <div className="text-center max-w-sm">
            ...
            <p className="text-sm text-muted-foreground">
              O motor de qualificação avalia automaticamente as 22 teses tributárias contra o perfil CNAE e regime da empresa.
            </p>
          </div>
        </section>
      )}
      ...
      <footer className="bg-primary-deep text-white/70 mt-12 py-5 text-xs">
        <div className="container text-center">
          Radar de Teses Tributárias · Hackathon Jurídico 2026 · Powered by Lovable AI · LC 214/2025 · Total catálogo: {TESES.length} teses
        </div>
      </footer>
    </div>
  );
